...
const links = [
  ...
  {
    to: "/dean/faculty-management",
    label: "Faculty Management",
    icon: <IoPeopleOutline className="w-5 h-5" />,
  },
  {
    to: "/dean/student-oversight",
    label: "Student Oversight",
    icon: <IoSchoolOutline className="w-5 h-5" />,
  },
  {
    to: "/dean/curriculum-planning",
...
const DeanSidebar = ({ className = "" }: SidebarProps) => {
      ...
      {!isDesktop && (
        <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-primary-50 backdrop-blur-md shadow">
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <div className="font-heading text-white text-xl">Dean</div>
            <span className="w-6" />
          </div>
        </div>
      )}
