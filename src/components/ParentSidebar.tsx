...
const ParentSidebar = ({ className = "" }: SidebarProps) => {
  ...
  return (
    <>
      ...
      {!isDesktop && (
        <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-primary-50 backdrop-blur-md shadow">
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <div className="font-heading text-white text-xl">Parent</div>
            <span className="w-6" />
          </div>
        </div>
      )}
      ...
      {/* Mobile top bar spacer - Rendered outside the flex row if possible, but here we just ensure it doesn't push horizontally */}
      {!isDesktop && <div className="h-[56px] w-0 flex-none" />}
    </>
  );
};
