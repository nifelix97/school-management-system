...
export default function TeacherMessage() {
    ...
    const initialConversations: Conversation[] = [
      ...
      },
      {
        ...
        lastMessage: {
          ...
          senderId: 3,
          receiverId: 1,
          content: "Thank you for the extra help with calculus!",
      ...
      {
        id: 5,
        participants: [currentUser, users.find(u => u.id === 6)!],
        lastMessage: {
          id: 7,
          senderId: 6,
          receiverId: 1,
  ...
  const getFilteredConversations = () => {
    ...
    switch (conversationFilter) {
      ...
      case "grade10a":
        filtered = conversations.filter(conv => {
          ...
          return otherParticipant.class === "Grade 10A";
        });
        break;
      case "grade10b":
        filtered = conversations.filter(conv => {
          const otherParticipant = getOtherParticipant(conv);
          return otherParticipant.class === "Grade 10B";
        });
        break;
      case "grade11a":
        filtered = conversations.filter(conv => {
          const otherParticipant = getOtherParticipant(conv);
          return otherParticipant.class === "Grade 11A";
        });
        break;
      default:
    ...
    if (conversationSearchQuery.trim()) {
      filtered = filtered.filter(conv => {
        const otherParticipant = getOtherParticipant(conv);
        return otherParticipant.name.toLowerCase().includes(conversationSearchQuery.toLowerCase()) ||
               conv.lastMessage.content.toLowerCase().includes(conversationSearchQuery.toLowerCase());
      });
    }
  ...
  const grade10aCount = conversations.filter(conv => {
    const otherParticipant = getOtherParticipant(conv);
    return otherParticipant.class === "Grade 10A";
  }).length;
  const grade10bCount = conversations.filter(conv => {
    const otherParticipant = getOtherParticipant(conv);
    return otherParticipant.class === "Grade 10B";
  }).length;
  const grade11aCount = conversations.filter(conv => {
    const otherParticipant = getOtherParticipant(conv);
    return otherParticipant.class === "Grade 11A";
  }).length;
                ...
                {filteredUsers.length === 0 ? (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs xs:text-sm text-gray-500">{user.class}</p>
                    </div>
                  </div>
                  ))
                )}
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-2 xxs:px-3 xs:px-4 py-3 xs:py-6">
        <div className="flex flex-col xxs:flex-row items-start xxs:items-center justify-between mb-4 xs:mb-6 gap-3 xxs:gap-0">
          <h1 className="text-lg xs:text-xl sm:text-2xl font-bold text-primary-50">Teacher Messages</h1>
          <button
          ...
          >
            ...
            <span className="xxs:inline">Message Student</span>
          </button>
        </div>
        ...
        <div className="flex gap-4">
          ...
          <div className="hidden sm:block w-48 bg-white rounded-lg shadow-sm p-4">
            ...
            <div className="space-y-1">
              ...
              <button
                onClick={() => {
                  setConversationFilter("grade10a");
                  setCurrentPage(1);
                }}
                className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg transition-colors ${
                  conversationFilter === "grade10a"
                    ? "bg-primary-50 text-white"
                    : "text-primary-50 hover:bg-gray-100"
                }`}
              >
                ...
                <span className="text-xs">{grade10aCount}</span>
              </button>
              
              <button
                onClick={() => {
                  setConversationFilter("grade10b");
                  setCurrentPage(1);
                }}
                className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg transition-colors ${
                  conversationFilter === "grade10b"
                    ? "bg-primary-50 text-white"
                    : "text-primary-50 hover:bg-gray-100"
                }`}
              >
                ...
                <span className="text-xs">{grade10bCount}</span>
              </button>

              <button
                onClick={() => {
                  setConversationFilter("grade11a");
                  setCurrentPage(1);
                }}
                className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg transition-colors ${
                  conversationFilter === "grade11a"
                    ? "bg-primary-50 text-white"
                    : "text-primary-50 hover:bg-gray-100"
                }`}
              >
                ...
                <span className="text-xs">{grade11aCount}</span>
              </button>
            </div>
          </div>
          ...
          <div className="sm:hidden w-full mb-4">
            ...
            >
              ...
              <option value="grade10a">Grade 10A ({grade10aCount})</option>
              <option value="grade10b">Grade 10B ({grade10bCount})</option>
              <option value="grade11a">Grade 11A ({grade11aCount})</option>
            </select>
          </div>
          ...
          <div className="divide-y divide-gray-200">
            {currentConversations.length === 0 ? (
              <div className="p-8 text-center text-primary-50">
                ...
                <p className="text-xs xs:text-sm mt-1">Start a new message to begin chatting with students</p>
              </div>
            ) : (
