...
interface AccountantProfileUpdate {
  firstName: string;
  lastName: string;
  email: string;
  telephoneNumber?: string;
...
type Props = {
  ...
  onUpdatePersonal?: (data: AccountantProfileUpdate) => Promise<void> | void;
};
...
const AccountantProfile: React.FC<Props> = ({
  profile,
  onUpdatePersonal,
}) => {
  ...
  const [form, setForm] = useState<AccountantProfileUpdate>({
  ...
  const mockProfile: AccountantProfile = {
    id: "acc001",
    firstName: "Sarah",
    lastName: "Johnson",
